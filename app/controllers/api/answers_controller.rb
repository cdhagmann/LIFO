...
module API
  class AnswersController < ApplicationController
    ...
    def create
      ...
      if @answer.save
        UserMailer.with(user: @answer.question.user,
                        url: question_url(@answer.question,
                                          anchor: "answer_#{@answer.id}")).alert_email.deliver_now
    ...
    def accept
      ...
      elsif @answer.update({ question_accepted_id: @answer.question.id })
    ...
    def reject
      ...
      elsif @answer.update({ question_accepted_id: nil })
        render :show, status: :ok, location: api_question_answer_url(question_id: @question.id, id: @answer.id)
      else
        render json: @answer.errors, status: :unprocessable_entity
      end
